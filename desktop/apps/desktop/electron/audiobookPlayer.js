'use strict'
// The audiobook player's arithmetic, kept apart from any page so it can be tested and so the
// website (audiobookApi.js handleWeb) can inline the very same functions with .toString().
// That is why every function here is SELF-CONTAINED: no reference to another function or to
// a constant outside its own body. The desktop app's React copy is src/lib/audiobookPlayer.js
// and test/audiobook-player.test.js checks that the two agree.
//
