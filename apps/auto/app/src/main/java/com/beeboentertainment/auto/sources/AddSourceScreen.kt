 ...
 *
 * It owns its own [SourcesRepository] and state, so it drops into the existing
 * settings screen with a single call and no plumbing:
 *
 *   // inside MainActivity.Screen(), e.g. below the certificate ToggleRow:
 *   HorizontalDivider()
 *   AddSourceScreen()
 *
 * Styling matches MainActivity: Material3, OutlinedTextField, Cards, the same
 * spacing. It adds no navigation and touches no player, so it is safe to insert
...
fun AddSourceScreen(
    ...
    val repo = remember { SourcesRepository(context) }
    ...
    Column(
        ...
        Text(
            "Paste a link to a video, audio file, playlist, or a JSON listing " +
                "you host yourself, and it becomes a source you can play.",
