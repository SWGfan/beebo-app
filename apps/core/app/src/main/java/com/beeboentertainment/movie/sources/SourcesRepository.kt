 ...
 * Local storage is the source of truth. The two hub-sync helpers at the bottom
...
class SourcesRepository(
    ...
    private val store = SourceStore(session)
    ...
    suspend fun addByUrl(raw: String): UserSource {
        val result = SourceProbe.probe(raw, http)
    ...
    suspend fun resolvePlayable(source: UserSource): PlayableRef = when (source.kind) {
        SourceKind.INDEX -> when (val r = SourceProbe.probe(source.url, http)) {
     ...
     * Best-effort: returns false and leaves local storage untouched when the route
