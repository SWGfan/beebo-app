 ...
 *
 * A slider from -500ms to +500ms bound to [SessionStore.audioDelayMs], with a
 * live label ("Video +120 ms"), fine -10/+10 nudge buttons for the last few
 * frames of alignment, and a Reset. The stored range in [SessionStore] is wider
 ...
 *
 * It writes straight to [SessionStore] on every change — [PartyController] reads
 * `session.audioDelayMs` fresh on each seek/sync, so a drag re-aligns the picture
...
fun AudioDelaySlider(
    ...
    var delay by remember { mutableIntStateOf(session.audioDelayMs) }

    fun update(newValue: Int) {
        val clamped = newValue.coerceIn(SLIDER_MIN, SLIDER_MAX)
        delay = clamped
        session.audioDelayMs = clamped
    ...
    Column(
        ...
        Text(
            "Viewers' video only. Nudge until lips match what the speakers play " +
                "— delayed audio arrives a little late, so a positive value holds " +
                "the picture back to meet it.",
