 ...
 * listing served by a streamServer-style box, or something we could not classify
 * but still let them keep. The list is persisted locally in SessionStore (see
 * [SourceStore]); the hub sync in [SourcesRepository] is a bonus on top.
 *
 * These models are @Serializable so the exact same shape round-trips through both
 * local storage JSON and the optional hub /api/v1/sources payload. Fields carry
 * defaults so an older stored blob keeps decoding as the type grows.
 */
